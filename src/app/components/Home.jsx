...
// Quick action card still uses Material Card
...
const Home = () => {
  ...
  useEffect(() => {
    const rootDiv = document.querySelector('[data-home-root]');
    if (rootDiv) {
      const isDark = ['slateDark1', 'slateDark2', 'slateDark3'].includes(activeTheme);
      rootDiv.style.backgroundColor = isDark ? '#121212' : '#ffffff';
    }
  }, [activeTheme]);
  ...
  return (
    ...
    >
      <StyledDiv>
        <h3
          style={{
            color: ['slateDark1', 'slateDark2', 'slateDark3'].includes(activeTheme)
              ? '#ffffff'
              : '#00246b',
          }}
        >
      ...
      <Grid container justifyContent="center" spacing={2} style={{ marginTop: '40px' }}>
        ...
        {user?.dynmis_empid && user.dynmis_empid !== '' && shouldShowSalarySlips() && (
          <Grid item xs={12} sm={4} md={3} lg={3}>
            <PayslipCard />
          </Grid>
        )}
