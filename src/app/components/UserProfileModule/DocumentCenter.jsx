...
const DocumentCenter = () => {
  ...
  // NEW: State for appointment letter section
  ...
  const fetchAppointmentLetter = async () => {
    try {
      ...
      // API call to get appointment letter details
  ...
  const handleAppointmentLetterDownload = () => {
    try {
      if (appointmentLetter && appointmentLetter.file_url) {
        ...
        link.setAttribute('target', '_blank'); // Add target blank for better compatibility
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
      } else {
  ...
  useEffect(() => {
    fetchData('CompanyPolicy');
    // NEW: Fetch appointment letter on component mount
