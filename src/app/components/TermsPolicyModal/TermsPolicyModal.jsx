  ...
  const handleAccept = async () => {
    ...
    if (currentDocIndex < documents.length - 1) {
      ...
      try {
        ...
        const errMsg =
          error?.response?.data?.Message || 'Failed to sync policy acceptance to server.';
    ...
    >
      ...
      <DialogContent>
        ...
        <DocumentHeader>
          ...
          {acceptedDocs[currentDoc.filename] && <CheckCircleIcon color="success" />}
        </DocumentHeader>

        {/* PDF Viewer */}
        <Box mb={2}>
          <PdfViewer src={currentDoc.path} title={currentDoc.title} />
        </Box>
          ...
          <FormControlLabel
            ...
            label={
              <Typography variant="body1">
                I have read and understood the <strong>{currentDoc.title}</strong> document and
                agree to comply with all the terms and policies mentioned therein.
              </Typography>
            }
          />
        </Paper>
      ...
      <DialogActions sx={{ p: 3, justifyContent: 'space-between' }}>
        <Button onClick={handlePrevious} disabled={currentDocIndex === 0} variant="outlined">
          Previous
        </Button>
